...
module.exports = function ScreenshotApp(options, callback) {
  ...
  const statics = {
    ...
    directWrite: true,
    webPreferences: {
      preload: join(__dirname, "screenshot-app-preload.js")
    }
  }

  let popupWindow = new BrowserWindow(deepAssign(defaults, options, statics))
  ...
  const makeScreenshot = () => {
    ...
    ipcMain.once(sizeEvent, (e, data) => {
      ...
      const width = Math.max(options.width, data.width)
      const height = Math.max(options.height, parseInt(data.height * (options.zoom || 1.0), 10))
